...
const ADMIN_MODULES = [
  ...
  { title: "Core Configuration", path: "/admin/settings", icon: Settings, desc: "API keys and maintenance", color: "text-text-secondary", ownerOnly: true },
];
    ...
    <div className="max-w-7xl mx-auto animate-fade-in-up">
      ...
      <div className="flex items-center justify-between mb-8 pb-6 border-b border-[#1a2236]">
         <div>
            ...
            <h1 className="font-display font-bold text-3xl text-text-primary tracking-widest uppercase">
               {userData?.isOwner ? 'OWNER CONTROL CENTER' : 'COMMAND CENTER'}
            </h1>
         </div>
         <div className="text-right font-mono text-[10px] text-text-muted tracking-widest uppercase">
            System Up Time<br/>
         ...
         {metrics.map((m, i) => (
            <div key={i} className="bg-[#0b0404] border border-[#1a2236] p-6 rounded-xl relative overflow-hidden group hover:border-primary/30 transition-all">
               ...
               <div className="flex justify-between items-start mb-4 relative z-10">
                  <span className="font-mono text-[10px] text-text-muted font-bold tracking-widest uppercase">{m.label}</span>
                  <m.icon size={18} className="text-primary" />
               </div>
               <div className="font-display font-bold text-3xl text-text-primary relative z-10">
      ...
      {/* MODULES GRID */}
      <div>
         <h2 className="font-mono text-[12px] font-bold text-text-muted tracking-widest uppercase mb-4">Control Matrices</h2>
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {modules.map((mod, i) => (
               ...
               >
                  ...
                  <h3 className="font-display font-bold text-lg text-text-primary tracking-wide mb-2">{mod.title}</h3>
                  <p className="font-mono text-[11px] text-text-secondary leading-relaxed tracking-widest uppercase">{mod.desc}</p>
               </Link>
            ))}
         </div>
      </div>
